...
优先于 cursor_vision.py（OCR）使用：
  - 不需要截屏、OCR、图像处理
  - 精度 100%，延迟 <100ms
  - 需要 Cursor 以 --remote-debugging-port=5253 启动
...
class CdpCursorState:
    ...
    # CDP 专有字段
    cdp_mode: bool = True
    cdp_lost: bool = False   # True = CDP 端口无响应（Cursor 崩溃/退出）
...
def scan(host: str = CDP_HOST, port: int = CDP_PORT) -> CdpCursorState:
    ...
    if not target:
        ...
        state.cdp_lost = True   # 端口无响应，标记 CDP 丢失
...
def _test():
    ...
    if not is_cdp_available():
        print("  × CDP 不可用")
        print("  请以 --remote-debugging-port=5253 启动 Cursor")
        print("  示例: Cursor.exe --remote-debugging-port=5253")
        sys.exit(1)
