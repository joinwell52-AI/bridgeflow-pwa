...
- Cursor 以 `--remote-debugging-port=5253` 启动时自动激活 CDP 模式（延迟<100ms，精度100%）
- CDP 不可用时自动降级到 OCR 模式（延迟1-3s，精度~95%）

## 团队模板：初始化后不可在面板内「切换」

- 团队种类（软件开发 / 自媒体 / MVP）在 **首次向导** 中写入项目下 **`fcop/codeflow.json`**。
- **已配置完成后，面板不提供「切换团队」按钮**。若要改用另一套模板，请使用面板顶部 **「重置」**（清除本机码流配置）后 **重新走向导** 选择项目与模板；详见面板「团队」区块说明与重置确认框。
- **「同步角色模板」**：仅按 **当前** `codeflow.json` 已选团队，把内置 `templates/agents/<team>/` 拷贝到项目的 `fcop/`（并清理根目录旧 `*.md`），**不会**单独把模板从 A 换成 B。

## 角色模板（多套团队）与主仓一致

内置三套：`templates/agents/dev-team/`、`media-team/`、`mvp-team/`。  
「同步角色模板」时：**先删除** `fcop/` **根目录下**旧有 `*.md`（不删 `tasks/` 等子目录与 `codeflow.json`），再拷贝当前团队目录下文件，避免换团队后开发组与自媒体角色文档混叠。  

dev-team 示例文件名：`PM-01.md`、`DEV-01.md`、`QA-01.md`、`OPS-01.md`、`ADMIN-01.md`（及 `.en.md`）。自媒体为 `PUBLISHER.md` 等，与面板「团队」一致。  

发布前请从仓库 **`fcop/`** 同步到 **`codeflow-desktop/templates/agents/dev-team/`**（本仓库已对齐；若只改主仓文档，请再执行一次复制以免 EXE 内仍是旧版）。
...
2. 若项目下无 `fcop/codeflow.json`，自动打开**沉浸式引导向导**（系统浏览器）：
...
> **`【码流巡检】巡检，开工。请自行查看 fcop/tasks/ 等待办任务。`**
...
- **CodeFlow 控制台**：管项目下 `fcop/` 任务文件、巡检、预检与轨迹。  
