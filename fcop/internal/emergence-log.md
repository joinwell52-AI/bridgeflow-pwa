...
| §2 | PM 自约束 1-10 + 9.1 派单后扩展 + 9.2 内容三件套版 |
| §3 | PM 错误自披露 1-20 |
...
| 9.2 内容三件套版 | 写 schema/protocol/integration 类 TASK 前，**必先 `Read` 相关 spike / 调研 / mapping 文档**（如 `p4-schema-mapping-v1.1.md`）。不靠 PM 心智记忆，因为「path 存在」≠「概念与 PM 心智对齐」 | DEV-017 D5-S1 揭示 + PM 第 19 次错误自披露（5/11 16:35 DEV 公开 surprise）|
...
| 19 | TASK-017 §2.1 schema 矩阵把 agent/review/skill 三共同 schemas 判为「**需 drift 核验**」 — 实际上 **3 个全是同名异义 / dual contract / 独立概念体系**（spike 文档 `p4-schema-mapping-v1.1.md` §2 早已证明）。PM 起草 TASK-017 时**没有回查 spike 文档**，沿用 TASK-007 §四（spike 之前 5/11 上午的 PM 心智）的「mirror drift」叙事 | 5/11 16:35 DEV-017 D5-S1 揭示 | DEV 自决用「路径 D」（spike 文档复验）完工 + 公开 surprise + 提出 path 三件套升级为「**内容三件套**」 |
| 20 | TASK-018 §2.1 stage 清单**漏列** `fcop/tasks/REPORT-20260511-016-OPS-to-PM.md`（OPS-016 完工回执，写于 commit `201b109` 之后故未入）— 如 OPS 直接执行，REPORT-016 会孤立留在 git status `??` 状态 | 5/11 17:23 PM 自约束 9.1 巡检 `git status` 时发现，OPS-018 启动前主动修正 | **自约束 9.1 第一次实战阻断** — PM 修正 TASK-018 §2.1 (7→8 项) + §3.2 流程 + §4 GATE 7 stage 清单。损失 0（OPS 未启动 TASK），节省 OPS-019 / Day 6 一次孤立补 REPORT 的代价 |

**模式归纳更新**：20 次错误中 **13 次（#6, 8, 9, 10, 11, 12, 13, 14, 15, 16, 18, 19, 20）属于「path/API/参数/内容/stage 清单」类**，**前 11 次** 由 path 三件套（v9）覆盖，**#19** 揭示 path 三件套**第 4 维度盲区**（内容对齐），**#20** 是首条**「派单后三件套（9.1）首次实战成功阻断」** — OPS 启动前 PM 巡检 `git status` 发现 stage 清单漏项，主动修正。**剩余 7 次**（#1, 2, 3, 4, 5, 7, 17）是判断 / 流程 / 战略 / 巡检节奏类，由 Charter 5/6 + 自约束 5/7/9.1/10 覆盖。

**DEV-017 提出的「内容三件套」**（新增第 4 维度 — 待 PM 接受入档为自约束 9.2）：

| # | 内容三件套维度 | 含义 | path 三件套 (v9) 覆盖度 | #19 失守点 |
|---|---|---|---|---|
| 1 | 文件 path 存在 | ✅ v9 已 cover | 100% | — |
| 2 | 文件可被解析（JSON parse / lint）| ⚠️ v9 部分 cover | 50% | — |
| 3 | **文件「概念」与 PM 心智模型对齐** | ❌ **v9 未 cover** | 0% | **#19 失守** |

**PM 接受 DEV 提议**：自约束 9 path 版升级到 **9.2 内容三件套版** — 写 schema/protocol/integration 类 TASK 前，**必先 `Read` 相关 spike / 调研文档**，不靠 PM 心智记忆。
...
**正面样本（自约束 9.1 首次实战阻断）**：#20 是「派单后三件套」诞生后第一次成功阻断 — PM 派完 TASK-018 后**未等 ADMIN 触发**，主动周期 `git status` + `git log` 自巡检，发现 stage 清单漏列 `REPORT-20260511-016-OPS-to-PM.md`（OPS-016 完工回执，应入 Day 5 commit）。**OPS 启动 TASK-018 前**修正 7 → 8 项，损失 0 + 节省 OPS-019 / Day 6 一次孤立补 REPORT 的代价。从 #17 到 #20 用时 40min（16:43 → 17:23），自约束 9.1 从「沉痛教训」到「主动阻断」的迭代闭环。
...
| I-5 | DEV-017 D5-S1 | **提出 path 三件套升级为「内容三件套」**：在「path 存在」之上加第 4 维度「文件概念与 PM 心智模型对齐」+ 强约束「派 schema/protocol/integration 类 TASK 前必先 Read spike 文档」— 催化 PM 第 19 次错误自披露 + 自约束 9.2 诞生 |
| I-6 | OPS-016 §六.1 | **stage 前 S0 类型救援**：发现 REPORT-015 原本含真实 `PYTHON_BIN=<local user Python path>` smoke 命令，OPS 主动改为占位符 `__REPLACE_WITH_YOUR_PYTHON_312_PATH__`，使第 6 项 private path gate 从将失败变为 0 match — 沉淀「OPS stage 前内容审计」最佳实践 |
