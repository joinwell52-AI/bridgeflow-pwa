...
| 9 path 版三件套 | 写 TASK 前必须 ripgrep 核对 path 存在 + public method 暴露 + 参数类型匹配 | PM 第 8/10/11/12/13/14 次错误自披露累积（5/11） |
...
**模式归纳**：14 次错误中 **8 次（#6, 8, 10, 11, 12, 13, 14, +9）属于「path/API/参数」类**，全部由第 9 条自约束 path 版三件套覆盖。**剩余 6 次** 是判断 / 流程 / 战略类，由 Charter 5/6 + 自约束 5/7/10 覆盖。
...
| D6 | fcop/ → fcop/ 路径迁移时机 | 待 ADMIN | DEV-005 §S8 证 fcop `workspace_dir="fcop"` 参数 PASS → 可推迟到 P5+ |
...
| DEV-011 Day 3 | 5/11 14:25 | ? | ? | 1.5d | 预期 8-10x |

**整 sprint 加速**：P4 关键路径累计 125min ≈ 2h，vs 原 PM SLA 2 工作日 ≈ **8-9x**。

若 Day 3-6 保持节奏 → **v0.3.0-alpha 5/12-5/13 EOD 出厂**（vs 原计划 5/17，提前 4-5 天）。PM **不承诺**（自约束 10），仅记录数据观察。
...
PM-01
2026-05-11 14:30 (UTC+8) — Day 3 进行中
