...
- 用途：分离协议体（markdown frontmatter）与磁盘编码（v1.0 fcop/ vs v0.x fcop/，ADR-0021）
- CodeFlow 当前：硬编码 `fcop/tasks/` 路径
...
1. **layout 默认是 `fcop/`，不是 CodeFlow 现用的 `fcop/`** —— D6 决策点尚未拍板（PM TASK §3.5 明令本调研不动这个）。`Project(path, workspace_dir="fcop")` 是 escape hatch。
...
| 阶段 | 工作 | 预估天数 |
|---|---|---|
| Day 1 | 跑 `Project.init(workspace_dir="fcop")` 在仓库副本上，diff 与 v0.1 fcop/ 现状（识别字段错配）| 0.5 d |
...
1. **D6（fcop/ vs fcop/）**：何时切换 layout？立即在 P4 切，还是 P4 维持 fcop/ 直到 P5？
...
| 发现 fcop 默认 layout=fcop/ 与 CodeFlow fcop/ 不一致 | **不**提议 fcop 调整，记入 §四「P4 维持 fcop/ via workspace_dir override」|
