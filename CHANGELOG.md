...
**背景**：过去三轮与多个 Agent 的策略对话中，一个锋利的观察浮现出来：

> FCoP 的协议表面 ~90% 长在**文件系统**上（文件名 40% + frontmatter 30% +
> 目录 20%），只有 ~10% 是行为约定。这让 FCoP 和 LangGraph / Temporal /
> CrewAI 这些**运行时框架**有物种级差别——前者"协议先存在，运行时只是
> 消费它"；后者"运行时先存在，协议后补"。

这个定位决定了一个重要设计原则：**MCP 工具箱（`fcop` 包）必须永远是
便利层，不能成为真相层**。否则协议会被藏进 Python 代码，失去"文件系统
即载体"的底层优势。

**改动**：

1. **新增对外定位长文** `docs/essays/fcop-position.md`（约 250 行）——
   把"协议先 vs 运行时先"的 2×2 历史类比（HTTP / Git / RSS / SMTP）、
   "文件名 40% + frontmatter 30% + 目录 20% + 行为 10%"的协议表面分解、
   "今天/主战场/远景"三层未来叙事、中立性护城河、工具化边界，完整展开
   成一篇可对外引用的架构论述。附 6 条维护者自校验清单。

2. **`fcop-protocol.mdc` 新增一节**"Architectural Principle: Tools are
   a Convenience Layer / 架构原则：工具是便利层，不是真相层"——插在
   Core Principle 之后、Rule 0.c 展开之前。正文包含三条推论：

   - **Consequence 1**：每个 MCP 工具的 docstring 必须写清它对应的几条
     基础文件操作（给出 `write_task` / `archive_task` / `list_tasks` /
     `read_task` / `new_workspace` 5 个示例）。如果某个工具无法被还原
     成文件操作，**就不得加入 `fcop` 包**
   - **Consequence 2**：设计检查点——没装 `fcop` MCP 的人（人或 Agent），
     用 `ls / cat / mv / git` 必须能完整参与协作。任何让这个答案变成
     "不能"的改动都是在往运行时里挪
   - **Consequence 3**：划清版本号语义——改文件名语法/frontmatter/目录/
     行为约定**必须**改 `fcop_protocol_version`；只改工具签名或错误文案
     **不改**。便利层演进可快，真相层演进须慎

   附 6 条自校验清单（每次演进必须全"是"才算健康）。

**意义**：这是 FCoP 从 0.x 走向 1.0 过程中最重要的一次"自我认知"固化。
之前的 0.x 版本在加工具，0.5.2 把"工具加法的边界"立了起来——以后每加
一个工具，都要回答"这个工具对应的文件操作是什么"，否则协议会渐渐被
藏进 `server.py` 里，变成另一个运行时框架。

这也对应着"把 FCoP 当标准打磨"的长线战略：HTTP / Git / RSS 之所以能
成为基础设施，是因为它们的协议独立于任何实现。FCoP 要走这条路，必须
现在就把"工具便利层"和"协议真相层"的边界讲清。

**零代码风险**：本次只改文档和协议说明；server.py 未动，所有现有工具
行为完全不变。
...
#### 修复：CDP click_role 侧栏不可见时用 scrollIntoView 再点击

- 顶部 tab 关闭时自动降级到右侧侧栏
- 侧栏元素不可见时先 scrollIntoView 再取坐标点击
...
#### 修复：CDP click_role 支持 tab 折叠场景（JS click 兜底）

- tab 不可见时用 `el.click()` 触发 React 合成事件，不依赖坐标
- 同时搜索 tab 栏、侧栏、agentTab class 三种 DOM 结构
...
#### 修复：切换前先恢复 Cursor 窗口，确保顶部 agent tab 可见

- CDP click_role 依赖顶部 tab 渲染，窗口最小化时 tab 不存在导致切换失败
- 切换前先 SW_RESTORE / SW_SHOW，再执行 CDP click
...
#### 修复：switch_agent_focus 改用 CDP click_role（和消息轮询完全一致，无需窗口焦点）
...
#### 修复：switch_agent_focus 完全复用切换实测逻辑

- CDP click_role → OCR/坐标点击，与切换实测代码路径完全一致，不再用命令面板
...
#### 修复：switch_agent_focus 增加命令面板第三通道

- **`nudger.py`**：切换 Agent 时增加第③通道——命令面板（`Ctrl+Shift+P` + 角色名）；不再依赖预检坐标或 CDP，任何情况都能切换
- 三条路优先级：① CDP → ② 坐标 → ③ 命令面板
...
#### 优化：版本升级时隐藏主操作区，改为展示产品宣传图

- **`panel/index.html` 升级展示区**：检测到新版本正在下载（`downloading`）或已下载完成（`ready`）时，主面板隐藏巡检控制等操作区域，改为展示产品宣传图（`product.png`），并在图下方显示升级提示文字
- 下载完成后提示文字自动更新为"新版本已下载完成"，顶部横幅同步显示"立即更新重启"按钮
