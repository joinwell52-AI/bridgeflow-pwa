...
|------|----------|---------|
| `codeflow_config.json` | PC working directory (generated by `codeflow init`) | All PC execution machine runtime parameters |
| `web/pwa/config.js` | PWA static resources | Phone-side relay address, cache version |

---

## 1. `codeflow_config.json` Full Parameter Dictionary

### Generation & Update

```powershell
codeflow init                          # First-time generation (defaults)
codeflow init --relay-url wss://...    # Specify relay URL during generation
codeflow init --room-key my-room       # Specify room key during generation
codeflow init --force                  # Force overwrite (⚠️ resets binding state)
```

When a config already exists, `codeflow init --relay-url/--room-key` only updates the corresponding field without affecting others.
...
|-------|------|---------|----------|-------------|
| `url` | `string` | `"wss://ai.chedian.cc/CodeFlow/ws/"` | ✅ | WebSocket relay URL. Must be `wss://` or `ws://`. Change when using self-hosted relay |
| `room_key` | `string` | `"CodeFlow-default"` | ✅ | Room isolation key. PC and phone must use the **exact same value** to communicate. Each team should use a unique random string |
| `shared_secret` | `string` | `""` | ❌ | Shared secret (reserved for future use, currently inactive) |

> **`room_key` Security Note:** The default `CodeFlow-default` is a public room — anyone can join. For team use, change it to a random string, e.g. `team-abc-2026-x7k`.
...
> **Note:** `root_dir` is auto-filled by `codeflow init` based on your current directory. No manual setup needed — works on any drive letter, any path.
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `runtime_dir` | `string` | `".CodeFlow/runtime"` | Runtime state file root (excluded from `.gitignore`) |
| `status_dir` | `string` | `".CodeFlow/runtime/status"` | Role status file directory (JSON files recording online/busy/idle state) |
| `task_details_dir` | `string` | `".CodeFlow/runtime/task_details"` | Task details cache directory (for dashboard and phone queries) |
...
## 3. `codeflow init` Command Parameters

| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `--project-root PATH` | `string` | Current directory | Root directory for config and task files |
| `--relay-url URL` | `string` | `wss://ai.chedian.cc/CodeFlow/ws/` | Override relay URL |
| `--room-key KEY` | `string` | `CodeFlow-default` | Override room key |
| `--force` | `flag` | `false` | Force regenerate config (⚠️ resets binding state and machine code) |

---

## 4. `codeflow run` Command Parameters

| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `--config PATH` | `string` | `./codeflow_config.json` | Specify config file path |
