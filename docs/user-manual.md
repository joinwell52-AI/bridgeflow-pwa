...
> v2.11.0 新增，v2.12.0 增强 Markdown 渲染与气泡对话样式

点击手机 PWA 团队卡片中的任意角色，即可打开该角色的**工作实况面板**，在手机上实时查看 PC 端 Agent 的对话内容。

### 效果对比

PC 端 Cursor 与手机端 PWA 工作实况内容保持一致：
...
| 消息气泡 | 显示 Agent 最近 20 条对话，用户/Agent 左右气泡区分 |
| Markdown 渲染 | 加粗、列表、代码块、标题等格式与 PC 端基本一致（v2.12.0+） |
| 队长标识 | 主控角色卡片右上角显示金色 ★ |
| 完整内容 | 每条消息最多传输 3000 字节，不再截断 |

### 使用方法

1. 手机 PWA 首页 → 点击团队卡片中任意角色
2. 工作实况面板自动打开，拉取 PC 端最新 Agent 状态
3. 下拉查看历史消息，格式与 PC 端 Cursor 对话一致
4. 点左上角 **← 返回** 关闭面板

### CDP 自动注入（零配置）

工作实况功能依赖 **Chrome DevTools Protocol（CDP）** 从 Cursor 内部直接读取对话内容，精度 100%，延迟 <100ms，无需截图或 OCR。

**CodeFlow Desktop 会全自动处理 CDP，用户无需任何额外配置：**
...
> **无需**手动修改快捷方式、命令行或任何配置文件。Desktop 启动时会自动完成检测和注入。

### 注意事项

- **只读监控**：面板底部标注"只读监控"，不能在此发送指令，请通过主界面发送任务
- **数据延迟**：约 5 秒推送一次，非严格实时
- **首次启动**：Desktop 首次检测到 Cursor 无 CDP 时会自动重启 Cursor，重启期间约需 10-15 秒，属正常现象

### 团队卡片说明

- 主控角色（队长）卡片显示 **★** 金色星标（如 mvp-team 的 MARKETER）
