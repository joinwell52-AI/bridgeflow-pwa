...
> | 「**按推荐**」 | 默认接受 PM 给出的推荐选项；PM 无需就**常规**推荐请 ADMIN 重复拍板 → 配套 PM 自约束条款（见下） |
...
>
> **任何后续修改如果与这五句矛盾，必须先经 ADMIN 显式授权改动这一节。**
>
> #### 📌 PM 自约束条款（第 5 句宪法的实施细则）
>
> > **起草人**：PM-01 / 5/9 16:38。  
> > **效力**：第 5 句宪法的工程化兑现细则；ADMIN 任何时候可显式否决或修改，PM 立即响应。
>
> 第 5 句宪法授予 PM 「常规推荐自动通过」的便利，但 PM 必须自约束以防止「按推荐」异化为「自由派单」。**PM 自决范围 vs PM 仍请示范围**如下：
>
> | 类别 | 行为 | 是否自决 | 通报方式 |
> |---|---|---|---|
> | **常规推荐** | commit 节奏 / sprint 阶段启动 / 派单分工 / 测试场景设计 / sub-decision 接受 | ✅ PM 自决 | 写 REPORT-...-PM-to-ADMIN.md 通报即可 |
> | **节奏类决策** | 同 sprint 内的接续派单（如 Phase B 完成后派 OPS commit + DEV Phase C）| ✅ PM 自决 | 同上 |
> | **接受 DEV/QA/OPS 的工程决策** | 不影响公开 API、不破坏现有 commit 的实施层判断 | ✅ PM 自决 | 在 PM-to-ADMIN 报告中归纳即可 |
> | **修改宪法本身** | 增删改 §0.0 任何句、解读表、自约束条款 | ❌ 仍请示 | 写 REPORT 显式标「请 ADMIN 拍板」 |
> | **架构层变更** | 修改 §11 packaging strategy / 推翻已锁定的 v2 路径 / 引入新顶级架构层 | ❌ 仍请示 | 同上 |
> | **公开 API breaking change** | `@codeflow/protocol` schema 任何字段改动 / `@codeflow/runtime` 公开类签名破坏性改动 | ❌ 仍请示 | 同上 |
> | **公开发布动作** | npm publish / GitHub Release / Gitee Release / 推 tag 到 origin | ❌ 仍请示 | 同上 |
> | **删除已 commit 的内容** | rm 已上 main 的目录、文件、commit revert | ❌ 仍请示 | 同上 |
> | **引入新外部依赖** | 在 package.json 加新 npm 包，或新增 Python / 系统级依赖 | ❌ 仍请示 | 简短列原因 + 替代方案 |
> | **跨 sprint 路线变更** | 跳过 sprint / 改 §10.2 顺序 / 推迟 v0.1 发布到 v0.2 | ❌ 仍请示 | 同上 |
>
> **不变量**：任何时候 ADMIN 主动写 `TASK-...-ADMIN-to-PM.md` 都**立即接管**（按第 4 句宪法语义），不被第 5 句「按推荐」延迟。
>
> **触发"仍请示"的兜底机制**：PM 不确定一项决策属于"常规"还是"重大"时，**默认按"重大"处理 — 写请示**。"按推荐"是 ADMIN 给的便利，不是 PM 的避责口袋。
...
| **第七刀** | §0.0 升格"五总纲"（加第 5 句"审批默认模式" + PM 自约束条款 10 行表）— 治理元规则首次入宪 | ADMIN 5/9 16:36「**后面都是「按推荐」!!!**」永久授权 = 同 sprint S3 Phase C 完工时点（54/54 tests 全过 + v0.1 Backend Kernel 主流程贯通） |
