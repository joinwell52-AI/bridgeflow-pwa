...
**标题:** 码流 CodeFlow v2.11 — 手机实时看 AI 在干什么，CDP 巡检 10ms 读 Cursor DOM（开源）

---

各位好！分享我做的开源项目：**码流（CodeFlow）v2.11.0**。

### 一句话介绍

用手机给 AI 团队发指令，PC 端自动执行，CDP 巡检 10ms 读 DOM，手机实时查看 Agent 在干什么，零数据库。

### v2.11 新功能：Agent 实时监控

手机点开任意 Agent 卡片 → 底部滑出实时面板，显示 Agent 最近 20 条消息摘要（分类：文字/代码/终端/文件编辑/工具调用/思考中），通过 WebSocket 实时推送，只读不干扰。
